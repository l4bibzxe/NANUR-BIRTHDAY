...
feedbackForm.addEventListener('submit', function(e){
  e.preventDefault(); // always stop the default redirect-away behavior now
  const actionUrl = feedbackForm.getAttribute('action') || '';

  if(actionUrl.includes('YOUR_FORM_ID')){
    feedbackStatus.textContent = "Not connected yet — swap the form's action URL for a real Formspree or Google Form link to make this live. Your message wasn't lost, just not sent: \"" + feedbackForm.message.value.slice(0,120) + (feedbackForm.message.value.length>120 ? '…' : '') + "\"";
    return;
  }
  ...
  .then(response => {
    if(response.ok){
      ...
      feedbackStatus.textContent = "Hmm, kono kechal hoise mone hoi?? Abar try kor";
    }
  })
  .catch(() => {
    feedbackStatus.textContent = "Couldn't send — tor connection check kor";
  })
  .finally(() => {
